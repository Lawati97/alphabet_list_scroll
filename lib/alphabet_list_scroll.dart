  ...
  _initAlphabetMap(String currentStr, int i) {
    ...
    if (currentStr.codeUnitAt(0) < 65 || currentStr.codeUnitAt(0) > 122) {
      ...
      heightMap["#"] = totalHeight;
        ...
        (currentStr.codeUnitAt(0) >= 65 && currentStr.codeUnitAt(0) <= 90)) {
      strMap[currentStr] = i;
...
class _AlphabetListScollViewState extends State<_AlphabetListScollView> {
  ...
  List<Widget> aToZ() {
    ...
    for (var x = 0; x < (widget.strList?.length ?? 0); x++) {
      Widget textview;

      if ((widget.strList?[x].length ?? 0) > 1) {
        var header =
      ...
      children: <Widget>[
        Center(
          child: Container(
            ...
            child: Padding(
              padding: const EdgeInsets.only(bottom: 10.0, left: 10, right: 10, top: 1),
